...
pDetection = aubio.pitch("default", 2048*5,
    2048//2, 44100)
# Set unit.
...
# pitch notification for player
SHRT_MAX=32767 # short uses 16 bits in complement 2 sample width

#oscillator creates wave value going from -1 to 1
def oscillator(time, frequency):
    radians = (time * 2.0 * math.pi * frequency)
    
    sin_value = math.sin(radians)
    return sin_value

#generate_tone makes tone with given duration
def generate_tone(duration = 1, wave_function = (lambda time: oscillator(time,1000))):  
    sample_rate = 44100
    sample_width=2
    ...
    for n in range(total_samples):
        time = n*sample_duration
        wave = int(SHRT_MAX*wave_function(time))
        data=struct.pack("h",wave)
        stream.write(data)

def create_wave_function(frequency=1000): #makes default 1sec wave
    return lambda time: oscillator(time,frequency)
...
#take pitch value in range from 100-350 and map it to y position 0-450
def pitch_transform(old_pitch):
    new_pitch = (((old_pitch - 50) * 450) / 300) + 0
    return new_pitch
...
def listen_to_speech():
    ...
    while not quit:
        ...
        try:
            ...
            # if recognizing easy 
            if recog_results == "easy" or recog_results == "level 1":
                level_1_id = True
                

    
            # if recognizing hard
            if recog_results == "hard" or recog_results == "level 2":
                level_2_id = True
               

            # if recognizing insane
            if recog_results == "insane" or recog_results == "level 3":
                level_3_id = True
                


            # if recogniozing menu
            if recog_results == "menu" or recog_results == "pause":
                menu_id = True
...
# -------------- Ball position: you can find it here -------
    ...
    def ball_pitch(self):
        ...
        b = self.ball
        #get y position pitch
        min_pitch = 70 #low detectable pitch
        generate_tone(wave_function=create_wave_function(min_pitch + b.y))
...
# -------------- Menu -------
    ...
    def update(self):
        ...
        if not self.paused:
            ...
            if ((pitch>= 100) and (pitch<= 400)): #change this to voice input
                p1_translation = pitch_transform(pitch) / 45
                print(p1_translation)
                if (p1_translation < 5):
                    p1.y -= (2.5*p1_translation)
                    p1.last_movements.append(+(2.5*p1_translation))
                elif (p1_translation > 5):
                    p1.y += p1_translation
                    p1.last_movements.append(-p1_translation)
            else:
...
# -------------------------------------#
#all_pitch_thread = threading.Thread(target=ball_pitch, args=())
#ball_pitch_thread.start()
